MODULE_NAME='Shure_DualReceiver_IP_Comm' (DEV vdvDevice, DEV dvDevice, CHAR devIP[])
...
DEFINE_FUNCTION fnGetShureRXQuadRep()
{
    ...
    WAIT 30 SEND_STRING dvDevice, " '< GET ',ITOA(IN_MIC_1),' BATT_BARS >' "
    WAIT 40 SEND_STRING dvDevice, " '< GET ',ITOA(IN_MIC_2),' BATT_BARS >' "

    WAIT 60 SEND_STRING dvDevice, " '< GET FW_VER >' "
    WAIT 70 SEND_STRING dvDevice, '< GET AUDIO_SUMMING_MODE >'
    WAIT 80 SEND_STRING dvDevice, '< GET DEVICE_ID >' //Can be 1-8 Characters Long...
    WAIT 90 SEND_STRING dvDevice, '< GET ENCRYPTION >' 
    WAIT 110 SEND_STRING dvDevice, '< GET MODEL >' //Needs FW 2.4+
}
