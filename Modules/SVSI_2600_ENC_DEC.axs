MODULE_NAME='SVSI_2600_ENC_DEC' (DEV vdvDevice, DEV dvDevice, CHAR devIP[])
...
VOLATILE LONG lTlIpConnect[] = {45000}; //45-second Query Loop...
...
DEFINE_FUNCTION fnStartSVSIEncDecConnection() {
	    ...
	    SEND_COMMAND vdvDevice, "'Attempt to Start SVSI Connection...'"
...
DATA_EVENT [vdvDevice]
{
    COMMAND :
    {
	STACK_VAR CHAR iMessage[100];
	    iMessage = DATA.TEXT;
	
	IF (FIND_STRING(iMessage, 'SWITCHAUDIO-',1)) {
		REMOVE_STRING (iMessage, '-',1)
			SEND_STRING dvDevice, "'seta:',iMessage,CR"
	    }
    }
}
DATA_EVENT [dvDevice]
{
	...
	WHILE (FIND_STRING(ResponseBuffer,"$0D",1))
	{
		...
		ACTIVE (FIND_STRING(iMsg, 'STREAMAUDIO:',1)) : {  
		    REMOVE_STRING (iMsg, ':',1)
			    SEND_COMMAND vdvDevice, "'FBAUDIOSTREAM-',iMsg"
		}
		ACTIVE (FIND_STRING(iMsg, 'STREAM:',1)) : {  // This is conflicting with finding AudioStream
		    REMOVE_STRING (iMsg, ':',1)
...
//		IF (bDbug == TRUE) {
//		    SEND_STRING 0, "'2600 : ',ResponseBuffer";
//		}
	    }
	}
    }
