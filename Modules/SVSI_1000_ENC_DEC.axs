MODULE_NAME='SVSI_1000_ENC_DEC' (DEV vdvDevice, DEV dvDevice, CHAR devIP[])
...
VOLATILE LONG lTlIpConnect[] = {20000}; //20-second Query Loop...
...
CHANNEL_EVENT [vdvDevice, 0]
{
	...
	SWITCH (CHANNEL.CHANNEL)
	{
	    ...
	    DEFAULT : {
		SEND_STRING dvDevice, "'set:',ITOA(CHANNEL.CHANNEL),CR"
	    }
	}
    }
}
DATA_EVENT [dvDevice]
{
	...
	STACK_VAR CHAR iName[25];
	STACK_VAR CHAR iPlaymode[10];
	STACK_VAR CHAR iStatus[15];
	...
	WHILE (FIND_STRING(ResponseBuffer,"$0D",1))
	{
		...
		ACTIVE (FIND_STRING(iMsg, 'NAME:',1)) : {
		    ...
		    iName = LEFT_STRING(iMsg,LENGTH_STRING(iMsg) -1) //Removes the $0D
			    SEND_COMMAND vdvDevice, "'FBSVSINAME-',iMsg"
		}
		ACTIVE (FIND_STRING(iMsg, 'STREAM:',1)) : {
		    REMOVE_STRING (iMsg, ':',1)
			    SEND_COMMAND vdvDevice, "'FBSVSISTREAM-',iMsg"
		}
		ACTIVE (FIND_STRING(iMsg, 'STREAMAUDIO:',1)) : {
		    REMOVE_STRING (iMsg, ':',1)
			    SEND_COMMAND vdvDevice, "'FBSVSIAUDIOSTREAM-',iMsg"
		}
		ACTIVE (FIND_STRING(iMsg, 'PLAYMODE:',1)) : {
			REMOVE_STRING (iMsg, ':',1)
			    iPlaymode = LEFT_STRING(iMsg,LENGTH_STRING(iMsg) -1) //Removes the $0D
			    SEND_COMMAND vdvDevice, "'FBSVSILIVE-',iPlaymode"
		}
		ACTIVE (FIND_STRING(iMsg, 'DVISTATUS:',1)) : {
		    REMOVE_STRING (iMsg, ':',1) // connected
			iStatus = LEFT_STRING(iMsg,LENGTH_STRING(iMsg) -1) //Removes the $0D
			    SEND_COMMAND vdvDevice, "'FBSVSISTATUS-',iStatus";
		}
	    }
	}
    }
